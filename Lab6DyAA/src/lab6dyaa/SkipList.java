...
public class SkipList<T extends Comparable<? super T>> {
    ...
    /**
     * Para una sesion interactiva lee datos desde el teclado
     *
     * @return dato que es un entero
     */
    ...
    /**Permite imprimir los primeros k elementos almacendados en el skiplist
     * @param k muestra los k primeros elementos en el skiplist
     */
    public void imprimir(int k) {

        int cont = 0;
        System.out.println("SkipList " + k + " primeros");
        //va bajando los niveles para recorrer el skiplist
        for (int i = maxLevel - 1; i >= 0; i--) {

            //imprime la lista nivel por nivel
            for (SkipListNode p = root[i]; p != null; p = p.next[i]) {

                System.out.print(p.key + " ");
                cont++;
                //si llega al k, termina el programa
                if (cont == k) {
                    System.exit(0);
                }
            }
